...
def add_col_uuid7_str(
    df: pl.DataFrame,
    col: str,
    overwrite: bool = False,
) -> pl.DataFrame:
    if ( not overwrite ) and col in df.columns:
        raise ValueError( "Already have column in df col={}".format( col ) )
    #
    return df.with_columns(
        pl.Series(
            name   = col,
            values = [ get_uuid7_str() for _ in range( df.shape[0] ) ],
            dtype  = pl.Utf8,
        )
    )
#/def add_col_uuid7_str
