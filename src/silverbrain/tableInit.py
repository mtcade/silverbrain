...
class TableInitRef():
    """
    A named table initializer. References a TaggedTableProcess by op_idn string
    for topological ordering and registration into tableOps.

    table_idn  — the primary table being initialized
    ...
    op_idn     — op_idn of the TaggedTableProcess in __table_processes__ that
                 initializes this table
    written_by — opIdns of post-init tableProcesses that update this table
    """
    table_idn:  str
    source:     tuple[str, ...]
    op_idn:     str
...
class ProcessInitRef():
    ...
    op_idn     — the op_idn under which the process will be registered; used to
                 check whether it is already present in __table_processes__
    source     — tables or other process op_idns that must be ready first
    factory_idn — op_idn of the factory TaggedTableProcess in __table_processes__;
                 when run it produces and registers the target process
    always_run — reserved for symmetry with TableInitRef; currently unused
    """
    op_idn:      str
    source:      tuple[ str, ... ]
    factory_idn: str | None
    always_run:  bool = False
...
def topological_init_order_from_df(
    ...
    for row in table_init_df.to_dicts():
        registry[ row[ 'table_idn' ] ] = ( 'table',   list( row[ 'source' ] ) )
    for row in process_init_df.to_dicts():
        registry[ row[ 'op_idn'   ] ] = ( 'process', list( row[ 'source' ] ) )
